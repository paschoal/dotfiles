let
  paschoal = "ssh-rsa  paschoal@asteros";
in {
  "wifi-networks.age".publicKeys = [ paschoal ];
  "wireguard-private-key.age".publicKeys = [ paschoal ];
  "wireguard-public-key.age".publicKeys = [ paschoal ];
  "wireguard-preshared-key.age".publicKeys = [ paschoal ];
}
